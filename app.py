...
def parse_gacha_input(input_string):
    ...
    return {
        ...
        "cardPoolId": get_first(["cardPoolId", "card_pool_id", "gacha_id"]),
    }


def fetch_gacha_records(server_id, player_id, language_code="zh-Hans", record_id="", card_pool_id=""):
    """
    调用鸣潮 API 获取抽卡记录
    遍历所有卡池类型，获取全部记录
    """
    ...
    for pool_type in POOL_TYPES:
        ...
        # 如果提供了 cardPoolId，则添加到请求中
        if card_pool_id:
            payload["cardPoolId"] = card_pool_id
...
def analyze():
    ...
    card_pool_id = params.get("cardPoolId", "")

    # 获取抽卡记录
    try:
        all_records = fetch_gacha_records(server_id, player_id, language_code, record_id, card_pool_id)
    except Exception as e:
